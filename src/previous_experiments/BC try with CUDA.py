...
imitation_policy = Imitation().to(device)
...
for episode in range(20):
    ...
    op_action_pred, op_action_prob, op_action_logit = imitation_policy(op_obs.to(device))

    correct_hold = []
    op_action_hold = []
    op_action_logit_hold = torch.Tensor().to(device)
    ...
    for t in range(40000):
        ...
        if op_action_real >= 0:
            ...
            op_action_logit_hold = torch.cat((op_action_logit_hold, op_action_logit.to(device)))
            correct_hold.append((op_action_real == op_action_pred).cpu())

        #Generate mixture:
        expert_logits = expert_policy.layers(e_obs)
        expert_probs = nn.Softmax(dim=0)(expert_logits.view(-1))
        imitation_logits = imitation_policy.layers(e_obs.to(device))
        ...
        prob_mixture = (1-scale)*expert_probs + scale*imitation_probs.detach().cpu()

        action = torch.distributions.Categorical(probs=prob_mixture).sample() + 1

        op_action_pred, op_action_prob, op_action_logit = imitation_policy(op_obs.to(device), deterministic = det)
    ...
    actions_stack = torch.stack(op_action_hold).to(device)
