...
assert len(sys.argv) == 5, "Please specify mixture weight, handicap, if frozen, and learning rate"
MIXTURE_WEIGHT = float(sys.argv[1])
HANDICAP = int(sys.argv[2])
FINETUNE = bool(int(sys.argv[3]))
LR = float(sys.argv[4])
BATCH_SIZE = 7000
N_BATCHES = 10

time = datetime.now().strftime('%m-%d-%H-%M')
output_dir = time+'_'+str(MIXTURE_WEIGHT)+'Transfer'+str(FINETUNE)
...
if FINETUNE:
    ...
    optimizer = torch.optim.Adam(imitation_policy.layers[2].parameters(), lr=LR)#0.0001 stable
else:
    optimizer = torch.optim.Adam(imitation_policy.parameters(), lr=LR)#0.0001 stable
